...
### Network
###
# Nginx Ingress GCE Global IP Reservation
resource "google_compute_global_address" "nginx_ingress_ip" {
  name    = "${var.nginx_ingress_ip_name_prefix}${local.env}"
...
resource "google_dns_record_set" "api" {
  ...
  type         = "A"
  rrdatas      = ["${google_compute_global_address.nginx_ingress_ip.address}"]
  ttl          = 300
  project      = var.project_id
}
...
# Output
output "nginx_ingress_ip_name" {
  value = google_compute_global_address.nginx_ingress_ip.name
}
output "nginx_ingress_ip_address" {
  value = google_compute_global_address.nginx_ingress_ip.address
}
